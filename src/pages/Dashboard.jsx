...
export default function Dashboard({ user: propUser }) {
  ...
  const POSITION_ORDER = ["Governor", "Senator", "Members of the County Assembly"];

  useEffect(() => {
    const fetchElections = async () => {
      try {
        const res = await API.get("/elections/");
        const sortedData = res.data.map((election) => {
          const sortedPositions = election.positions.sort(
            (a, b) =>
              POSITION_ORDER.indexOf(a.title) - POSITION_ORDER.indexOf(b.title)
          );
          return { ...election, positions: sortedPositions };
        });
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {elections.map((election) => (
          <section
            key={election.id}
            className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-10"
          >
            ...
            <div className="p-6 space-y-10">
              {/* Render each position separately */}
